...
void selecaoDireta(tipoVetor V, int ini, int fim)
{
    int i, j, min, aux;
...
void* selecaoThread(void *arg)
{
    Param *p = (Param*) arg;
    selecaoDireta(A, p->ini, p->fim);
    return NULL;
}
...
void* mergeSortThread(void *arg)
{
    ...
    if (ini < fim)
    {
        ...
        else
        {
            ...
            pthread_create(&t1, NULL, selecaoThread, &p1);
            pthread_create(&t2, NULL, selecaoThread, &p2);
